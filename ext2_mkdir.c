...
//length of the file
long file_byte;
//char array the stores the name of the file with padding.
char *pad_name;
//char array that stores the char array.
unsigned char *block_bm;
//length of padding for the file
int pad_len;
int file_entry_len;
//new record length
...
int fn_len;
//Count for the number of tokens in the path.
int token_count;
//char array for filename
char *fn;
struct ext2_inode *working_inode;
...
unsigned get_inode_num(struct ext2_inode *ip, char *name){
  ...
  int len=0;
  while(len<1024) {
    if(strncmp(db->name,name, db->name_len) == 0) {
      if (GETTFILETYPE(db)== 'd' )
      {
        return db->inode;
      }
    }
...
void init_new_entry(struct ext2_dir_entry_2 *entry,unsigned int node_num,char *name ){
...
int main(int argc, char **argv) {
  ...
  /* find the parent parsed path */
  if (token_count == 0)
  {
    fn = path_token[token_count];
    fn_len = strlen(fn);
    pad_file_name(fn);
  }else{
fn = path_token[token_count];
fn_len = strlen(fn);
  ...
  /* get the inode refered by the path */
  
  if (token_count ==0)
  {
    working_inode = root_inode;
  }else{
    working_inode = get_inode(root_inode, inode_bm, ip, parent_path_token);
  }
  unsigned int parent_inode_number = get_inode_num(working_inode,".");
  unsigned int check_node = get_inode_num(working_inode,fn);
  if (check_node != 0)
   {
     perror("EEXIST");
    exit(1);
 ...
 /*get free inode position  */
  ...
  //Initialize the . entry and .. entry

  struct ext2_dir_entry_2 *dot_entry = malloc(12);
  struct ext2_dir_entry_2 *ddot_entry= malloc(12);
  init_new_entry(dot_entry,inode_entry_num,"." );
  init_new_entry(ddot_entry,parent_inode_number,".." );

  /* get the  free entry position*/
  struct ext2_dir_entry_2 *free_entry = get_free_entry_pos(working_inode);

/* initialize the entry */
  int memcpysize = sizeof(struct ext2_dir_entry_2)+pad_len; 
  struct ext2_dir_entry_2 *new_dir = malloc(memcpysize);
  new_dir->inode = inode_entry_num;
  new_dir->rec_len = new_rec_len;
  new_dir->name_len = strlen(fn);
  new_dir->file_type = 2;
...
ibo = 0;
   ...
   memcpy(to_data_block, dot_entry,12);
   to_data_block+=12;
   memcpy(to_data_block, ddot_entry,12);
